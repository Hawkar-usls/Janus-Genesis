...
These tests intentionally describe the hardened contract before the SIM-2 router
implements it. They use only local fixtures and mocked responses. The module is
expected to fail against the frozen pre-repair router and must not be enabled in
required green CI until the laboratory records that pre-repair failure state.
"""
...
class RouterHardeningExpectedFailures(unittest.TestCase):
    ...
    def test_a06_duplicate_case_id_must_be_accounted_not_accepted_twice(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(router, "fetch_bytes", return_value=GOOD_BYTES):
