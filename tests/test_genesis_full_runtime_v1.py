...
class GenesisFullRuntimeV1Tests(unittest.TestCase):
    ...
    def test_public_contract_is_exact_historical_mirror(self):
        self.assertEqual(self.public, self.contract)
        self.assertEqual(self.contract["schema"], "janus.genesis.full_runtime.v1")
    ...
    def test_r02_browser_artifacts_survive_supersession(self):
        for path in (WORLD, BRIDGE, MATERIAL, AUDIO):
            self.assertTrue(path.exists())
        self.assertIn("GENESIS_WORLD_RUNTIME_V3", self.world)
    ...
    def test_historical_krr_save_does_not_store_rendered_binary(self):
        ...
        for token in ("download_pointer", "new Image()", "KRR", "materialDNA"):
