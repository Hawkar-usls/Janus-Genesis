#!/usr/bin/env python3
"""Exact reducer for the R50G32 remaining two-occurrence role quotient."""
...
REQUEST_ID = "TRUMP-R50G32-GENERAL-TWO-OCCURRENCE-NONSWAP-011"
EXPECTED_CHUNKS = 32
...
def main() -> int:
    ...
    if pair_paths != EXPECTED_PAIR_PATHS or len(unique_pairs) != EXPECTED_PARENT_STATES:
        raise AssertionError("R50G32_REDUCER_PARENT_SEAL_DRIFT")
    if dict(rule_partition) != EXPECTED_RULE_PARTITION:
        raise AssertionError("R50G32_REDUCER_PARENT_PARTITION_DRIFT")

    files = sorted(Path(args.chunks_dir).rglob("chunk-*.json"))
    if len(files) != EXPECTED_CHUNKS:
        raise AssertionError(("R50G32_REDUCER_CHUNK_FILE_COUNT", len(files), EXPECTED_CHUNKS))
    ...
    if set(by_index) != set(range(EXPECTED_CHUNKS)):
        raise AssertionError(("R50G32_REDUCER_CHUNK_INDEX_SET", sorted(by_index)))

    ordered_hashes = [h for h, _ in sorted(unique_pairs.items())]
    coverage: list[str] = []
    chunk_manifest: list[dict[str, Any]] = []
    raw_paths = within_chunk_unique = duplicate_paths = 0
    open_count = closed_count = audited_edges = missing_edges = 0
    raw_patterns: Counter[str] = Counter()
    first_seen_patterns: Counter[str] = Counter()
    ...
    file_by_index = {int(json.loads(p.read_text(encoding="utf-8"))["chunk_index"]): p for p in files}
    for idx in range(EXPECTED_CHUNKS):
        r = by_index[idx]
        if int(r["chunk_count"]) != EXPECTED_CHUNKS:
            raise AssertionError(("R50G32_REDUCER_CHUNK_COUNT_DRIFT", idx, r["chunk_count"]))
        if tuple(r.get("declared_remaining_role_patterns", [])) != tuple(r50g32.PATTERNS):
            raise AssertionError(("R50G32_REDUCER_ROLE_PATTERN_DRIFT", idx))
        if int(r.get("histogram_l1_drift", -1)) != 4:
            raise AssertionError(("R50G32_REDUCER_HISTOGRAM_DRIFT", idx))
        expected = [h for pos, h in enumerate(ordered_hashes) if pos % EXPECTED_CHUNKS == idx]
        got = list(r["selected_parent_hashes"])
        if got != expected:
            raise AssertionError(("R50G32_REDUCER_PARENT_PARTITION_MISMATCH", idx))
        coverage.extend(got)
        any_resource_limit |= bool(r["resource_limit_reached"])
        raw_paths += int(r["raw_valid_general_nonswap_path_count"])
        within_chunk_unique += int(r["within_chunk_unique_mutant_count"])
        ...
        add_counter(raw_patterns, r["raw_role_pattern_partition"])
        add_counter(first_seen_patterns, r["within_chunk_first_seen_role_pattern_partition"])
        ...
        for sample in r.get("all_r47j_pivots_closed_samples", []):
            if len(witness_samples) < 32:
                witness_samples.append(sample)
        chunk_manifest.append({
            "chunk_index": idx,
            "parent_state_count": len(got),
            "receipt_sha256": digest_file(file_by_index[idx]),
            "raw_paths": int(r["raw_valid_general_nonswap_path_count"]),
    ...
    if len(coverage) != EXPECTED_PARENT_STATES or len(set(coverage)) != EXPECTED_PARENT_STATES:
        raise AssertionError("R50G32_REDUCER_PARENT_COVERAGE_NOT_EXACT")
    all_chunks_exhaustive = all(bool(r["chunk_exhaustive_within_declared_remaining_two_occurrence_grammar"]) for r in receipts)
    if open_count + closed_count != within_chunk_unique:
        raise AssertionError("R50G32_REDUCER_AUDIT_ACCOUNTING_DRIFT")
    if set(raw_patterns) - set(r50g32.PATTERNS):
        raise AssertionError(("R50G32_REDUCER_UNKNOWN_PATTERN", sorted(set(raw_patterns)-set(r50g32.PATTERNS))))

    if any_resource_limit or not all_chunks_exhaustive:
        status = "UNKNOWN_RESOURCE_LIMIT"
        next_gate = "R50G32_RECHUNK_GENERAL_TWO_OCCURRENCE_NONSWAP_REPLAY"
        answer = "At least one R50G32 chunk did not exhaust its exact parent partition. Rechunk the identical remaining two-occurrence role quotient; no finite-negative claim is permitted."
    elif closed_count:
        status = "ALL_R47J_PIVOTS_CLOSED_WITNESS_CANDIDATE_FOUND"
        next_gate = "R50G33_ALL_R47J_PIVOTS_CLOSED_GENERAL_REWIRE__DIRECT_R33_RUP_AFFINE_OTHER_FROZEN_OPERATOR_AUDIT"
        answer = f"R50G32 exactly covered all 1424 frozen parent states and found {closed_count} all-R47J-pivots-closed within-chunk audits among the remaining two-occurrence role patterns. Next: direct frozen-operator audit; no P-vs-NP conclusion."
    else:
        status = "FINITE_NEGATIVE_WITHIN_COMPLETE_TWO_OCCURRENCE_REWIRE_ROLE_QUOTIENT"
        next_gate = "R50G33_TWO_OCCURRENCE_REWIRE_EXHAUSTED__MINIMUM_THREE_OCCURRENCE_ROLE_CYCLE"
        answer = f"R50G32 exactly covered all 1424 frozen parent states across {EXPECTED_CHUNKS} exhaustive chunks. All {within_chunk_unique} within-chunk unique audits in the four remaining L1=4 role patterns retained a replay-verified R47J door. Together with R50G30 reciprocal swaps and R50G31 one-relay rewires, the declared two-occurrence sign-preserving role quotient is finite-negative. Cross-chunk formula uniqueness is intentionally not claimed."

    receipt = {
        "schema": "janus.genesis.trump_research_receipt.v1_11",
        "lane_id": "JANUS_TRUMP_R50G32_GENERAL_TWO_OCCURRENCE_ROLE_QUOTIENT_LANE",
        ...
        "report": {
            ...
            "partition_rule": "SORTED_PAIR_HASH_POSITION_MOD_32",
            "all_chunks_exhaustive": all_chunks_exhaustive,
            "resource_limit_reached": any_resource_limit,
            "remaining_role_patterns": list(r50g32.PATTERNS),
            "histogram_l1_drift": 4,
            "raw_valid_general_nonswap_path_count": raw_paths,
            "raw_role_pattern_partition": dict(raw_patterns),
            "within_chunk_unique_formula_audit_count": within_chunk_unique,
            "within_chunk_first_seen_role_pattern_partition": dict(first_seen_patterns),
    ...
    print(f"R50G32_AGGREGATE_RECEIPT={out}")
    return 0
