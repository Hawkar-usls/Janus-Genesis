#!/usr/bin/env python3
"""Proof-carrying chunk worker for the frozen R50G31 minimum nonswap grammar."""
...
import trump_r50g31_two_occurrence_nonswap_rewire_agent as r50g31
...
def run(workspace: Path, chunk_index: int, chunk_count: int) -> dict[str, Any]:
    if chunk_count <= 0 or not (0 <= chunk_index < chunk_count):
        raise ValueError("BAD_CHUNK_COORDINATES")
    ...
    if pair_paths != EXPECTED_PAIR_PATHS or len(unique_pairs) != EXPECTED_UNIQUE_PAIR_STATES:
        raise AssertionError(("R50G31_CHUNK_PARENT_COUNT_DRIFT", pair_paths, len(unique_pairs)))
    if dict(unique_partition) != EXPECTED_UNIQUE_RULE_PARTITION:
        raise AssertionError(("R50G31_CHUNK_PARENT_PARTITION_DRIFT", dict(unique_partition)))
    ...
    for pair_hash, row in selected:
        if resource_limited:
            break
        for edit in r50g31.nonswap_load_transfers(r50g23, row["formula"]):
            raw_paths += 1
            ...
            for pivot in vars_:
                ...
                if not replay.get("pass"):
                    raise AssertionError(("R50G31_CHUNK_REPLAY_FAIL", chunk_index, mh, pivot))
            ...
            if found is None:
                ...
                if len(closed_samples) < MAX_SAMPLES:
                    closed_samples.append({
                        ...
                        "classification": "ALL_R47J_PIVOTS_CLOSED_R50G31_CHUNK_WITNESS",
                    })
            else:
                any_open += 1
    ...
    if any_open + all_closed != len(seen):
        raise AssertionError("R50G31_CHUNK_ACCOUNTING_DRIFT")
    return {
        "schema": "janus.genesis.trump_r50g31_chunk_receipt.v1",
        ...
        "duplicate_valid_path_count": duplicates,
        ...
        "chunk_exhaustive_within_declared_grammar": not resource_limited,
        "truth_boundary": {
...
def main() -> int:
    ...
    print(f"R50G31_CHUNK_RECEIPT={args.output}")
    return 0
