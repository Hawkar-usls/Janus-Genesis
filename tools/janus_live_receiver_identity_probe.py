...
def process_record(proc_root: Path, pid: int) -> dict:
    ...
    return {
        ...
        "pid_namespace": safe_link(base / "ns" / "pid"),
    }
...
def parse_proc_tcp(path: Path, port: int) -> list[dict]:
    ...
    for line in text.splitlines()[1:]:
        cols = line.split()
        if len(cols) < 10 or cols[3] != "0A" or ":" not in cols[1]:
            continue
...
def namespace_listener_owners(proc_root: Path, anchor_pid: int, port: int) -> dict:
    ...
    net_ns = safe_link(anchor / "ns" / "net")
    listeners = parse_proc_tcp(anchor / "net" / "tcp", port) + parse_proc_tcp(anchor / "net" / "tcp6", port)
    listener_inodes = {item["inode"] for item in listeners}
    owners: list[dict] = []
    if net_ns and listener_inodes:
        for pid in iter_pids(proc_root):
            if safe_link(proc_root / str(pid) / "ns" / "net") != net_ns:
                continue
            overlap = sorted(listener_inodes & socket_inodes_for_pid(proc_root, pid))
            if overlap:
...
def find_git_provenance(path: Path) -> dict | None:
    # Keep /proc/<pid>/root lexical. Resolving this magic symlink can escape
    # the target process root and accidentally inspect the host namespace.
    current = path
    try:
        if current.is_file():
    ...
    for candidate in [current, *current.parents]:
        provenance = parse_git_head(candidate)
        if provenance:
            return provenance
    return None
...
def docker_rows(names: tuple[str, ...]) -> list[dict]:
    ...
    for line in proc.stdout.splitlines():
        ...
        if not any(service.lower() in name.lower() for service in names):
            continue
        try:
...
def evaluate(receipt: dict, expected_owner: str | None, expected_commit: str | None) -> dict:
    exact_source_commits: set[str] = set()
    ...
    for namespace in receipt["port_8008_namespaces"]:
        for owner in namespace.get("owners", []):
            owner_names.add(" ".join([owner.get("comm") or "", *(owner.get("cmdline") or [])]))
        if namespace.get("listeners"):
            owner_names.update(namespace.get("docker_names") or [])

    source_state = "EXACT_SOURCE_IDENTITY_OBSERVED" if exact_source_commits else "UNRESOLVED"
    if expected_commit:
        source_state = "EXPECTED_COMMIT_MATCH" if expected_commit in exact_source_commits else "EXPECTED_COMMIT_NOT_PROVEN"

    any_listener = any(namespace.get("listeners") for namespace in receipt["port_8008_namespaces"])
    port_state = "LISTENER_OBSERVED_OWNER_UNRESOLVED" if any_listener else "UNRESOLVED"
    if owner_names:
