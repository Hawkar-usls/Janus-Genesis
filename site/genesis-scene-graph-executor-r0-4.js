(() => {
  ...
  const OPERATIONS=Object.freeze({
    STRUCTURE:new Set(['GENERATE_STRUCTURE']),
    ENTITY:new Set(['SPAWN_ENTITY','SPAWN_GROUP']),
    MATERIAL:new Set(['RESOLVE_MATERIAL']),
    ATMOSPHERE:new Set(['SET_ATMOSPHERE']),
    TERRAIN:new Set(['WORLD_TRANSFORM']),
    SOUND:new Set(['AUDIO_CUE']),
    EFFECT:new Set(['RUINED_TOWER']),
    PRESENTATION:new Set(['SET_CAMERA','SET_MIRROR','SET_CAMERA_DISTANCE']),
    RULE:new Set(['SPATIAL_RELATION']),
    INSPECTION:new Set(['INSPECT','VISIBLE_FAILURE']),
    NAVIGATION:new Set(['MOVE','RETURN_TO_HEARTH','PLACE_MARK'])
  });
  const FORBIDDEN_PARAM_KEY=/^(?:eval|code|source_code|script|shell|shell_command|exec|executable|module|import|require|filesystem|file_path)$/i;
  ...
  function canonicalValue(v){if(Array.isArray(v))return v.map(canonicalValue);if(isObject(v)){const out={};for(const key of Object.keys(v).sort())out[key]=canonicalValue(v[key]);return out;}return v;}
  function canonicalJson(v){return JSON.stringify(canonicalValue(v));}
  async function canonicalHash(v){return sha256(canonicalJson(v));}
  ...
  function validateAuthority(authority){if(!isObject(authority)||authority.janus_graph_is_world_authority!==false||authority.model_output_is_command!==false||authority.genesis_validator_required!==true||authority.world_mutation_authorized_by_janus!==false||authority.external_effect_authorized!==false)throw new Error('SCENE_GRAPH_AUTHORITY_BOUNDARY_INVALID');}
  function validateParams(value,depth=0){
    if(depth>8)throw new Error('SCENE_GRAPH_PARAM_DEPTH_LIMIT');
    if(value===null||typeof value==='boolean')return;
    if(typeof value==='number'){if(!Number.isFinite(value))throw new Error('SCENE_GRAPH_PARAM_NUMBER_INVALID');return;}
    if(typeof value==='string'){if(value.length>1000)throw new Error('SCENE_GRAPH_PARAM_STRING_LIMIT');return;}
    if(Array.isArray(value)){if(value.length>64)throw new Error('SCENE_GRAPH_PARAM_ARRAY_LIMIT');for(const item of value)validateParams(item,depth+1);return;}
    if(isObject(value)){const keys=Object.keys(value);if(keys.length>48)throw new Error('SCENE_GRAPH_PARAM_OBJECT_LIMIT');for(const key of keys){if(key.length>64)throw new Error('SCENE_GRAPH_PARAM_KEY_LIMIT');if(FORBIDDEN_PARAM_KEY.test(key))throw new Error('SCENE_GRAPH_ARBITRARY_CODE_FIELD_FORBIDDEN');validateParams(value[key],depth+1);}return;}
    throw new Error('SCENE_GRAPH_PARAM_TYPE_FORBIDDEN');
  }
  ...
  function validateGraph(graph){
    ...
    for(const node of graph.nodes){
      if(!isObject(node)||!/^n-[0-9a-f]{16}$/.test(String(node.id||'')))throw new Error('SCENE_GRAPH_NODE_ID_INVALID');
      if(byId.has(node.id))throw new Error('SCENE_GRAPH_NODE_ID_DUPLICATE');
      const family=String(node.family||'');if(!FAMILIES.has(family))throw new Error('SCENE_GRAPH_FAMILY_NOT_ALLOWLISTED');
      const operation=String(node.operation||'');if(!operation||operation.length>64)throw new Error('SCENE_GRAPH_OPERATION_INVALID');
      if(!OPERATIONS[family]?.has(operation))throw new Error('SCENE_GRAPH_OPERATION_NOT_ALLOWLISTED');
      if(typeof node.concept!=='string'||node.concept.length>160)throw new Error('SCENE_GRAPH_CONCEPT_INVALID');
      if(typeof node.required!=='boolean')throw new Error('SCENE_GRAPH_REQUIRED_FLAG_INVALID');
      if(!isObject(node.params))throw new Error('SCENE_GRAPH_PARAMS_INVALID');validateParams(node.params);
  ...
  async function verifyTransport(response){
    ...
    validateAuthority(response.authority);
    if(!isObject(response.scene_graph))throw new Error('SCENE_GRAPH_BODY_INVALID');
    const proof=response.transport_proof;if(!isObject(proof)||typeof proof.canonical_graph_json!=='string'||!hex64(proof.canonical_graph_utf8_sha256))throw new Error('SCENE_GRAPH_TRANSPORT_PROOF_REQUIRED');
    if(!globalThis.crypto?.subtle)throw new Error('SCENE_GRAPH_WEBCRYPTO_REQUIRED');
    const digest=await sha256(proof.canonical_graph_json);if(digest.toLowerCase()!==String(proof.canonical_graph_utf8_sha256).toLowerCase())throw new Error('SCENE_GRAPH_TRANSPORT_HASH_MISMATCH');
    let parsed;try{parsed=JSON.parse(proof.canonical_graph_json);}catch{throw new Error('SCENE_GRAPH_TRANSPORT_JSON_INVALID');}if(!structuralEqual(parsed,response.scene_graph))throw new Error('SCENE_GRAPH_TRANSPORT_STRUCTURE_MISMATCH');
    const graphCore={...response.scene_graph},graphClaim=String(graphCore.graph_hash||'');delete graphCore.graph_hash;if(!hex64(graphClaim))throw new Error('SCENE_GRAPH_HASH_REQUIRED');const graphComputed=await canonicalHash(graphCore);if(graphComputed.toLowerCase()!==graphClaim.toLowerCase())throw new Error('SCENE_GRAPH_CANONICAL_HASH_MISMATCH');
    const responseCore={...response},receiptClaim=String(responseCore.receipt_hash||'');delete responseCore.receipt_hash;if(!hex64(receiptClaim))throw new Error('SCENE_GRAPH_RECEIPT_HASH_REQUIRED');const receiptComputed=await canonicalHash(responseCore);if(receiptComputed.toLowerCase()!==receiptClaim.toLowerCase())throw new Error('SCENE_GRAPH_RESPONSE_CANONICAL_HASH_MISMATCH');
    return validateGraph(response.scene_graph);
  }

  function collectAssetRefs(node,receipts){const refs=[];for(const dep of node.depends_on||[]){const receipt=receipts.get(dep);if(Array.isArray(receipt?.asset_refs))refs.push(...receipt.asset_refs);}return refs.slice(0,8);}
  ...
  function intentFromNode(node){const op=String(node.operation||''),p=node.params||{},concept=node.concept||p.concept;
    ...
    if(node.family==='ATMOSPHERE'&&op==='SET_ATMOSPHERE')return{kind:'SET_ATMOSPHERE',time:p.time,fog:p.fog,weather:p.weather};
  ...
  async function executeNode(node,context){
    ...
    if(node.family==='EFFECT'&&node.operation==='RUINED_TOWER'){
      const intent={kind:'GENERATE_STRUCTURE',concept:'ruined tower',structure_kind:'tower',placement:'IN_FRONT_OF_PLAYER',distance:6,action_seed:context.graph.action_seed};const out=rt.executeIntent(intent,{receipt_hash:context.janus_receipt_hash||null,asset_refs:collectAssetRefs(node,context.receipts)});return out?.ok?{ok:true,result:{fallback:'GENERATE_RUINED_TOWER',mutation:out.mutation||null}}:{ok:false,reason:out?.reason||'EFFECT_REJECTED'};
    }
    if(node.family==='ENTITY'&&node.operation==='SPAWN_GROUP'){
      const count=Math.max(1,Math.min(8,Number(p.count)||3)),made=[],failures=[];for(let i=0;i<count;i++){const out=rt.executeIntent({kind:'GENERATE_STRUCTURE',concept:`${node.concept} ${i+1}`,structure_kind:'tree',distance:3.2+i*.72,action_seed:`${context.graph.action_seed}:${i}`},{receipt_hash:context.janus_receipt_hash||null,asset_refs:collectAssetRefs(node,context.receipts)});if(out?.ok)made.push(out.mutation||true);else failures.push({index:i,reason:out?.reason||'GENESIS_VALIDATOR_REJECT'});}if(failures.length)return{ok:false,reason:`GROUP_PARTIAL_FAILURE:${made.length}/${count}`,result:{requested:count,materialized:made.length,failures,appearance:p.appearance||null,placement:p.placement||null}};return{ok:true,result:{count:made.length,appearance:p.appearance||null,placement:p.placement||null}};
    }
    ...
    const out=rt.executeIntent(intent,{receipt_hash:context.janus_receipt_hash||null,asset_refs:collectAssetRefs(node,context.receipts)});return out?.ok?{ok:true,result:out}:{ok:false,reason:out?.reason||'GENESIS_VALIDATOR_REJECT'};
  }
  ...
  async function executeGraph(graph,{janus_receipt_hash=null,resolveMaterial=null,source='UNKNOWN'}={}){
    ...
    for(const id of validated.order){const node=validated.byId.get(id),blocking=(node.depends_on||[]).find(dep=>{const depNode=validated.byId.get(dep),r=receipts.get(dep);return depNode?.required!==false&&r&&r.status!=='APPLIED';});let status='FAILED',reason=null,result=null,asset_refs=[];
      if(blocking){status='SKIPPED_DEPENDENCY';reason=`FAILED_REQUIRED_ANCESTOR:${blocking}`;}else{try{const out=await executeNode(node,context);asset_refs=out?.asset_refs||[];result=out?.result??null;if(out?.ok)status='APPLIED';else{status='FAILED';reason=out?.reason||'NODE_FAILED';}}catch(e){status='FAILED';reason=e?.message||String(e);}}
  ...
  globalThis.GENESIS_SCENE_GRAPH_EXECUTOR_R0_4=Object.freeze({version:VERSION,limits:LIMITS,validateGraph,verifyTransport,executeGraph,structuralEqual,canonicalHash});
})();