        ...
        *,
        ...
        if self._intent(action) == "invent" and result.status == "GOOD_REALIZED":
            return replace(
                ...
                narrative=(
                    result.narrative
                    + "\n\nСтарый слой заметил в поступке доброе изменение, но Genesis не свёл свободную фразу к готовой категории или меню. Неизвестный замысел остался самостоятельным действием пути."
                ),
            )
        return result
